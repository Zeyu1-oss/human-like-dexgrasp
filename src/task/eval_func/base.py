...
class BaseEval:
    ...
    def _eval_pene_and_contact(self):
        ...
        self.cnum=len(contact_link_set)
    ...
    def run(self):
        ...
        if self.configs.task.simulation_metrics is not None:
            ...
            min_cnum = getattr(self.configs.task, "min_contact_num", 1)
            succ_cpoint_path = self.input_npy_path.replace(
                self.configs.grasp_dir, self.configs.succ_cpoint
            )
            if (
                eval_results["succ_flag"]
                and eval_results.get("contact_num", 0) >= min_cnum
                and not os.path.exists(succ_cpoint_path)
            ):
                os.makedirs(os.path.dirname(succ_cpoint_path), exist_ok=True)
                os.system(
                    f"ln -s {os.path.relpath(self.input_npy_path, os.path.dirname(succ_cpoint_path))} {succ_cpoint_path}"
                )

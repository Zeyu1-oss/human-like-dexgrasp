...
def read_data(npy_path):
    try:
        ...
        data["file_name"] = os.path.basename(npy_path)
        return data
    except Exception as e:
        print(f"[WARNING] Failed to load {npy_path}: {e}")
        return None  # 或 return {}，或者 raise 自定义异常
...
def get_diversity(data_lst):
    ...
    return pca.explained_variance_ratio_[0] * 100  # Diversity 
...
def task_stat(configs):
    ...
    data_lst = [d for d in data_lst if d is not None]  # 关键过滤

    print(f"[DEBUG] Loaded {len(data_lst)} grasps")

    data_success = [d for d in data_lst if d.get("succ_flag", False)]


    if getattr(configs, "scale_fig", False):
        save_path = os.path.join(configs.log_dir, "objscale_distribution.png")
        draw_obj_scale_fig(data_lst, save_path)

    if getattr(configs, "roc_fig", False):
        if "qp_metric" not in data_lst[0]:
            logging.warning("Please set 'eval_analytic' and 'eval_simulate' to True while evaluation")
        else:
    ...
    if "ho_pene" not in data_lst[0]:
        ...
        average_contact_number_for_succ = np.mean([d["contact_num"] for d in data_success])
        logging.info(f"Contact number for succ: {average_contact_number_for_succ}")       
    ...
    txt_save_path = os.path.join(configs.log_dir, "contact_numbers.txt")
    with open(txt_save_path, "w") as f:
        for idx, d in enumerate(data_lst):
            contact_num = d.get("contact_num", "N/A")
            file_name = d.get("file_name", f"sample_{idx}")
            f.write(f"{file_name}: contact_num = {contact_num}\n")

    logging.info(f"Saved contact numbers to {txt_save_path}")

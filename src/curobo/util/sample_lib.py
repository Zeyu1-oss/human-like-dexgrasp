...
class HaltonGenerator:
    ...
    def _get_samples(self, num_samples: int):
        if self._sample_buffer is not None:#_sample_buffer where did they come from
            out_buffer = None
    ...
    def get_samples(self, num_samples, bounded=False):
        samples = self._get_samples(num_samples)#get random samples from _sample_buffer
        if bounded: #true

...
class ArmCostConfig:
    ...
    def _get_base_keys():
        k_list = {
            ...
            "joint_consistency_cfg": JointConsistencyConfig,
        }
        return k_list
...
class ArmBase(RolloutBase, ArmBaseConfig):
    ...
    def _init_after_config_load(self):
        ...
        if (
            hasattr(self.constraint_cfg, "joint_consistency_cfg")
            and self.constraint_cfg.joint_consistency_cfg is not None):
            joint_cfg = self.constraint_cfg.joint_consistency_cfg

    # 关节名转 index：用 state_bounds.joint_names
            ...
            self.joint_consistency_constraint = JointConsistency(joint_cfg)
        else:
            self.joint_consistency_constraint = None
    ...
    def constraint_fn(
     ...
     # ✅ 新增 joint consistency constraint
        if self.joint_consistency_constraint is not None:
             joint_state = state.state_seq.position
             joint_constraint = self.joint_consistency_constraint.forward(joint_state)
             constraint_list.append(joint_constraint)
    ...
    def get_metrics(self, state: Union[JointState, KinematicModelState]):
        ...
        """
        if self.cuda_graph_instance:#pass
        ...
        out_metrics.cost, _ = self.cost_fn(state)#forces
        return out_metrics
    ...
    def _get_augmented_state(self, state: JointState) -> KinematicModelState:
        ...
        """
        Augment the robot's kinematic state with a time (horizon) dimension.

        If the input state is single-step (shape [B, D]), this function adds a
        dummy horizon dimension (H=1) to all relevant fields, producing shape [B, 1, D].

        This ensures compatibility with action sequences of shape [B, H, d_action].

        Args:

           state (JointState): Input joint state.

        Returns:
            KinematicModelState: Horizon-augmented kinematics state.
        """
    ...
    def compute_kinematics(self, state: JointState) -> KinematicModelState:
        ...
        #Compute the kinematic state of the robot given a joint configuration
        h = 0
    ...
    def rollout_fn(self, act_seq, opt_progress=1.0, debug_flag=False) -> Trajectory:
        ...
        with profiler.record_function("cost/all"):#armreacher.cost_fn
            cost_seq, debug = self.cost_fn(state, act_seq, opt_progress=opt_progress, debug_flag=debug_flag)#800 cost/output related to force

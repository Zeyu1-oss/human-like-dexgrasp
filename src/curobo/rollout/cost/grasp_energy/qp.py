...
class QPEnergy(GraspEnergyBase):
    ...
    def construct_Q_matrix(self, grasp_matrix, target_wrenches):
        batch_num, point_num = grasp_matrix.shape[:2] #场景接触点数量
        ...
        grasp_matrix_with_target = torch.cat([grasp_matrix, - repeated_target_wrench], dim=-1) # [bm, 6, 4n+1] Gx-W
        Q_matrix = grasp_matrix_with_target.transpose(-2, -1) @ grasp_matrix_with_target  # [bm, 4n+1, 4n+1] Q=(Gx-W)*(Gx-W)^T
    ...
    def forward(self, pos, normal, target_wrenches):#底层优化能量

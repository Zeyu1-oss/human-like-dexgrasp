...
class GraspSolverConfig:
    ...
    def load_from_robot_config(
        ...
        aux_rollout = ArmReacher(grad_cfg)#用于cost_fn
        ...
        if use_gradient_descent:#true 
            ...
            newton_cfg = NewtonOptConfig(**newton_d)#配置好
        ...
        if use_particle_opt:
            ...
            opts = []
        opts.append(lbfgs)
        cfg = WrapConfig(  #组合rollout+optimizer+cost_fn
            safety_rollout=aux_rollout,#cost
            optimizers=opts,#optimizers list
            compute_metrics=True,
            use_cuda_graph_metrics=grad_config_data["lbfgs"]["use_cuda_graph"],
            sync_cuda_time=sync_cuda_time,
        )
        grasp = WrapBase(cfg)#封装
        ...
        q_sample_gen = HeurGraspSeedGenerator( #生成初始种子
...
class GraspSolver(GraspSolverConfig):
    ...
    def update_goal_buffer(
        ...
        q_sample = self.sample_configs(self.world_coll_checker.n_envs)#[0, 0, 0, 0, ..., 0]
        kin_state = self.fk(q_sample)
        goal_pose = Pose(kin_state.ee_position, kin_state.ee_quaternion)#根据机器人当前可达性生成的一个“初步姿态目标”
        ...
        return self._goal_buffer #目标位姿、收缩参考配置、目标状态
    ...
    def solve_batch_env(
        ...
        retract_config: Optional[T_BDOF] = None,
        seed_config: Optional[T_BDOF] = None,
        return_seeds: int = 1,#return seeds=20
        num_seeds: Optional[int] = None,
        use_nn_seed: bool = True,
        newton_iters: Optional[int] = None,
        link_poses: Optional[Dict[str, Pose]] = None,
        ...
        if num_seeds is None:
            num_seeds = self.num_seeds
        if return_seeds > num_seeds: #return seeds=20
            num_seeds = return_seeds

        solve_state = ReacherSolveState(
            ReacherSolveType.BATCH_ENV,
            num_ik_seeds=num_seeds,#初始抓取姿态种子数量

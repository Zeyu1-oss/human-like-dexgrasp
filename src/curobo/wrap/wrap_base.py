...
class WrapBase(WrapConfig):
    ...
    def optimize(self, act_seq: torch.Tensor, shift_steps: int = 0) -> torch.Tensor:
        for opt in self.optimizers:
            act_seq = opt.optimize(act_seq, shift_steps)#opt_base,act_seq=seed
        return act_seq
    ...
    def update_params(self, goal: Goal):
        with profiler.record_function("wrap_base/safety/update_params"):
            ...
            self.safety_rollout.update_params(goal)#update goal?
    ...
    def solve(self, goal: Goal, seed: Optional[torch.Tensor] = None):
        ...
        filtered_state = self.safety_rollout.filter_robot_state(goal.current_state)# filteed_state=jointstate(positon...)
        goal.current_state.copy_(filtered_state)# update goal，
        self.update_params(goal) #update goal?
        if seed is None:
            seed = self.get_init_act()
            log_info("getting random seed")
        else:
            seed = seed.detach().clone() #seed
        ...
        act_seq = self.optimize(seed, shift_steps=0)#best-q
        self.opt_dt = time.time() - start_time

        act = self.safety_rollout.get_robot_command( #get one of the results -act
            filtered_state, act_seq, state_idx=goal.batch_current_state_idx
        )

        if self.compute_metrics:
            with profiler.record_function("wrap_base/compute_metrics"):
                metrics = self.get_metrics( #armreachermetrics
                    act, self.use_cuda_graph_metrics #act=jointstate
                )  # TODO: use cuda graph for metrics

        debug_info = self.get_debug_data()

        result = WrapResult(
            action=act,#joint state
            solve_time=self.opt_dt,
            metrics=metrics,#contact frame/force/point cost 
            debug=debug_info,#cost mid_result dist_error
            raw_action=act_seq,
        )
        return result
